...
# Model names
EMBED_MODEL = "sentence-transformers/all-MiniLM-L-6-v2"
...
# --- 6. Helper Function for Data Loading (Using PyMuPDF) ---
...
def load_and_index_files(_uploaded_files: Tuple[st.runtime.uploaded_file_manager.UploadedFile, ...]) -> Optional[
    ...
    all_docs = []
    print(f"load_and_index_files: Received {len(_uploaded_files)} files to process.")  # DEBUG LOG

    with st.spinner("Parsing and chunking documents..."):
        for f in _uploaded_files:  # Iterate over the file objects
            name = f.name
            data = f.read()  # Read the bytes *inside* the cached function

            print(f"Processing file: {name}")  # DEBUG LOG
            txt = load_text_from_bytes(name, data)  # Pass raw bytes
            if not txt:
                print(f"Skipping file {name} as no text was extracted.")  # DEBUG LOG
                continue
    ...
    if not all_docs:
        print("No documents were successfully chunked.")  # DEBUG LOG
        return None
...
def retrieve(query: str, k: int, _index: faiss.Index, _embedder: SentenceTransformer, _all_docs: List[Dict]) -> List[
    Dict]:
...
def build_messages(query: str, context_chunks: List[Dict], tone: str, role: str,
                   asked_accessibility: bool) -> List[Dict]:
...
if data:
    ...
    if uploaded_files:
        # This error is now triggered if load_and_index_files returns None
